...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  ...
  /// Get statistics from lost items only (items reported and active lost cases)
  Map<String, int> _getLostItemStatistics(List<PostedItem> items) {
    final activeLost = items
        .where(
          (item) =>
    ...
    return {'total': items.length, 'activeLost': activeLost};
  }
  ...
  Future<void> _confirmDelete(
    ...
    final canDelete = canModifyItem(item.status, item.type);
      ...
      body: RefreshIndicator(
        ...
        child: profileAsyncValue.when(
          ...
          error: (error, stack) => ListView(
            physics: const AlwaysScrollableScrollPhysics(),
            children: [SizedBox(height: 300, child: _buildErrorState(error))],
          ),
          data: (profile) {
            ...
            return postedItemsAsyncValue.when(
              ...
              data: (postedItems) {
                ...
                // Get statistics from lost items and user's claims
                final lostStats = _getLostItemStatistics(postedItems);
                final claimStatsAsync = ref.watch(userClaimStatisticsProvider);
                                    ...
                                    children: [
                                      SizedBox(
                                        ...
                                        child: _StatCard(
                                          ...
                                          value: lostStats['total']!.toString(),
                                          color: AppTheme.primaryBlue,
                                        ),
                                      ),
                                      SizedBox(
                                    ...
                                    children: [
                                      ...
                                      SizedBox(
                                        ...
                                        child: _StatCard(
                                          icon: Icons.hourglass_top_outlined,
                                          label: 'Claims Pending',
                                          value: claimStats['claimsPending']!
                                              .toString(),
                                          color: AppTheme.warningOrange,
                                        ),
                                      ),
                                      SizedBox(
                                        ...
                                        child: _StatCard(
                                          icon: Icons
                                              .assignment_turned_in_outlined,
                                          label: 'Awaiting Pickup',
                                          value: claimStats['awaitingPickup']!
                                              .toString(),
                                          color: AppTheme.successGreen,
                                        ),
                                      ),
